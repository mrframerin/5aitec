"""Shorten the ShopOS body on public/work/ehealth-arena/index.html."""
import os, re
...
# New shorter body
new_full = (
    "Full-stack AI operating system for D2C and Shopify brands. ARIA "
    "orchestrates a named agent roster - each owning a domain across "
    "content, ad copy, product launches, and CRM workflows - while Brand "
    "Memory means the AI actually knows the brand, not just the prompt. "
    "Backed by Binny Bansal (Flipkart co-founder); design partners include "
    "Agilitas, Celio, Lenovo, and Brevo."
).encode('utf-8')
...
def length_aware_replace(raw_bytes, old, new):
    ...
    if remaining:
        ...
        print(f"  outside-chunk replacements: {remaining}")
    return out
